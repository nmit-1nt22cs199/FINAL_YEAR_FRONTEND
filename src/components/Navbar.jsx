...
export default function Navbar({ currentPage, setPage, onLogout }) {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50">
      ...
      {/* Navigation Content */}
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-4 flex items-center justify-between">
        {/* Logo Section */}
        ...
        <button
          ...
          className="md:hidden p-2 rounded-lg bg-slate-900/40 border border-cyan-500/20 text-cyan-400 hover:bg-slate-800/50 transition-all duration-300"
        >
      ...
      {mobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-slate-950/98 backdrop-blur-md border-t border-cyan-500/20 shadow-xl">
          <div className="max-w-7xl mx-auto px-4 py-4 space-y-2">
            {navItems.map((item) => (
              <button
                key={item.id}
                onClick={() => handleNavClick(item.id)}
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg font-medium transition-all duration-300 text-sm ${currentPage === item.id
                  ? 'bg-gradient-to-r from-cyan-500 to-blue-500 text-white shadow-lg shadow-cyan-500/50'
            ...
            <button
              ...
              className="w-full flex items-center gap-3 px-4 py-3 bg-red-500/10 border border-red-500/30 rounded-lg text-red-400 hover:bg-red-500/20 transition-all duration-300 text-sm font-medium"
            >
